    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-teal-50 to-white">
        ...
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-semibold tracking-tight">
              {pendingMode ? 'Upload Documents (Pending)' : 'Student Application'}
            </h1>
          </div>
        </div>
      </header>
      ...
      <main className="px-4 sm:px-6 py-6">
        <div className="max-w-4xl mx-auto space-y-6">
          {/* Alerts */}
          {pendingMode && (
            <div className="border rounded-xl p-4 bg-amber-50/80 border-amber-200 text-amber-900 shadow-sm">
              <div className="font-semibold">Your application is pending</div>
              <div className="text-sm mt-1">
                Please upload required documents. You can also view what you already uploaded below.
              </div>
          ...
          {rejectedMode && (
            <div className="border rounded-xl p-4 bg-rose-50/80 border-rose-200 text-rose-900 shadow-sm">
          ...
          <section className="bg-white/80 backdrop-blur-xl rounded-2xl border border-white/60 shadow-[0_8px_30px_rgba(0,0,0,0.08)] p-6">
            ...
            {(pendingMode || existing?.exists) && (
              <div className="mb-6">
                <h2 className="text-sm font-semibold text-slate-900 mb-2">Current Documents</h2>
                <ul className="space-y-2">
                  {DOCS_LIST.map(([label, key]) => {
                    ...
                    return (
                      <li key={key} className="flex items-center justify-between border rounded-xl px-3 py-2 bg-white/70">
                        <div className="flex items-center gap-3 overflow-hidden">
                          ...
                          <div className="min-w-0">
                            <div className="text-slate-900 text-sm font-medium truncate">{label}</div>
            ...
            <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Basic fields */}
              <div>
                <label className="block text-sm font-medium mb-1 text-slate-800">Your Name</label>
                <input
                  name="name"
                  className="w-full border rounded-lg px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
                  value={form.name}
                  onChange={onChange}
                  required
                  disabled={(!canEditInPending && pendingMode) || rejectedMode}
                />
              </div>
              <div>
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium mb-1 text-slate-800">Father Name</label>
                <input
                  name="fatherName"
                  className="w-full border rounded-lg px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
              ...
              <div>
                <label className="block text-sm font-medium mb-1 text-slate-800">CGPA</label>
                <input
                  name="cgpa"
                  type="number"
                  step="0.01"
              ...
              <div>
                <label className="block text-sm font-medium mb-1 text-slate-800">Father Income</label>
                <input
                  name="fatherIncome"
                  type="number"
                  min="0"
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium mb-1 text-slate-800">Department</label>
                <select
                  name="department"
                  className="w-full border rounded-lg px-3 py-2 bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500"
              ...
              {/* Documents (keep enabled in pending mode) */}
              <div className="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-800">Aadhar Card (PDF/Image)</label>
                  <input type="file" name="aadharCard" accept=".pdf,image/*" onChange={onFileChange} className="w-full border rounded-lg px-3 py-2 bg-white" disabled={rejectedMode} />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-800">Income Certificate (PDF/Image)</label>
                  <input type="file" name="incomeCertificate" accept=".pdf,image/*" onChange={onFileChange} className="w-full border rounded-lg px-3 py-2 bg-white" disabled={rejectedMode} />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-800">Resume (PDF)</label>
                  <input type="file" name="resume" accept=".pdf" onChange={onFileChange} className="w-full border rounded-lg px-3 py-2 bg-white" disabled={rejectedMode} />
                </div>
                <div>
                  <label className="block text-sm font-medium mb-1 text-slate-800">Result Sheet (PDF/Image)</label>
                  <input type="file" name="resultsheet" accept=".pdf,image/*" onChange={onFileChange} className="w-full border rounded-lg px-3 py-2 bg-white" disabled={rejectedMode} />
                </div>
              </div>

              {error && <div className="md:col-span-2 text-sm text-rose-700 bg-rose-50 border border-rose-200 rounded-lg p-2">{error}</div>}
              {success && <div className="md:col-span-2 text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-lg p-2">{success}</div>}

              <div className="md:col-span-2">
                <button
                  disabled={loading || rejectedMode}
                  className="w-full bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-700 hover:to-teal-700 text-white font-semibold rounded-lg px-4 py-2 disabled:opacity-60"
                >
